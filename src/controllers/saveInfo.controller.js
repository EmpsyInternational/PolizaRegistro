...
controller.saveUser = async (req, res) => {
    try {
        const { cedula_rif, nombre, tipo_aliado, telefono, email, direccion, password } = req.body;
        ...
        if (user.code === 200) {
            ...
            let result = await saveUser({ cedula_rif, nombre, tipo_aliado ,telefono, direccion, email, password });
